...
<body>
	<div data-role="page" id="order" class="tx-order">
		...
		<div data-role="main" class="mt61">
			<form id="txOrderForm" action="${CONTEXT_PATH}/txOrderCmt1" method="post" data-ajax="false">
				<input type="hidden" name="order.deliverytype" value="1"/>
				<input type="hidden" id="seedTypeList" name="seedTypeList" value="${seedTypeList!}"/>
				<input type="hidden" id="proList" name="proList" value="${proList!}"/>
				<input type="hidden" id="seedTypeId" name="seedTypeId" value="${seedTypeId!}"/>
				<input type="hidden" id="amount" name="amount" value="${amount!}"/>
				<input type="hidden" id="exchangeId" name="exchangeId" value="${exchangeId!}"/>
				<input type="hidden" id="exchangeType" name="exchangeType" value="${exchangeType!}"/>
	...
	<script>
		...
		$(function(){
			...
			$("#txCmt").on("click",function(){
				var data={seedTypeList:$("#seedTypeList").val(),proList:$("#proList").val(),exchangeId:$("#exchangeId").val(),exchangeType:$("#exchangeType").val()};
   		     $.ajax({
		            type: "POST",
		            url: "${CONTEXT_PATH}/isExchangeOrder",
		            data: data,
		            dataType: "json",
