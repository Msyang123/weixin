$(document).ready(function() {
    ...
    $('body').on('click','.initials ul li',function() { 
            var _this=$(this);
            var LetterHtml=_this.html();
            LetterBox.html(LetterHtml).fadeIn();
            _this.css({'background':'rgba(105, 200, 237,0.4)',
                'color':'#fff'
            	});
            _this.siblings().css({
            	'background':'rgba(105, 200, 237,0)',
            ...
            if(_index==0){
            	$('#A').css('color','rgb(105,200,237)');
            	$('#A').siblings().css('color','rgb(153,153,153)')
                $('html,body').animate({scrollTop: '0px'}, 300);//点击第一个滚到顶部
            }else if(_index==26){
            	$('#default').css('color','rgb(105,200,237)');
            	$('#default').siblings().css('color','rgb(153,153,153)')
                var DefaultTop=$('#default').position().top;
                $('html,body').animate({scrollTop: DefaultTop+'px'}, 300);//点击最后一个滚到#号
            }else{
                var letter = _this.text();
                if($('#'+letter).length>0){
                	$('#'+letter).css('color','rgb(105,200,237)');
                	$('#'+letter).siblings().css('color','rgb(153,153,153)')
        ...
        var InitHeight=windowHeight-75;
