...
// 转化为整数
    ...
    Parabola.prototype = {
        ...
        initialize: function (options) {
            ...
            this.elOriginalLeft = ops.el&&ops.el!=""?toInteger(this.$el.offset().left):0;//toInteger(this.$el.css("left"));
            this.elOriginalTop = ops.el&&ops.el!=""?toInteger(this.$el.offset().top):0;//toInteger(this.$el.css("top"));
            // this.driftX X轴的偏移总量
            //this.driftY Y轴的偏移总量
            if (ops.targetEl) {
                this.driftX = toInteger(_$(ops.targetEl).offset().left) - this.elOriginalLeft+ops.offset[0];
                this.driftY = toInteger(_$(ops.targetEl).offset().top) - this.elOriginalTop+ops.offset[1];
            } else {
        ...
        getOptions: function (options) {
            ...
            options = $.extend({}, defaultSetting, (options.el?_$(options.el).data():null), (this.options || {}), options);
        ...
        start: function (goodsInfo,pfId) {
            ...
            this.pfId=pfId;
            //显示隐藏的小圆点
            this.$el.css("filter","alpha(opacity=1)")
            .css("-moz-opacity","1")
        ...
        reset: function (x, y) {
            ...
            x = x ? x : 0;
            y = y ? y : 0;
            this.elOriginalLeft+=x;
            this.elOriginalTop+=y;
            this.$el.css({
                position: "absolute",
                left: this.elOriginalLeft,
                top: this.elOriginalTop,
